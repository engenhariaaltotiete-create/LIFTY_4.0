...
function parseOneWorkoutText(text:string){
 ...
 if(!parsedBlocks.length)throw new Error('Treino sem subblocos.')
...
export async function importLiftyText(text:string){
 ...
 for(const part of parts){
  ...
  for(let bi=0;bi<parsed.blocks.length;bi++){
   ...
   if(!pb.values.NAME)throw new Error('Subbloco sem nome.')
  ...
  const id=await db.workouts.add({name:parsed.root.NAME,description:parsed.root.DESCRIPTION,formatVersion:4,blocks,archived:false,createdAt:now,updatedAt:now});ids.push(id)
 }
 return ids
}
